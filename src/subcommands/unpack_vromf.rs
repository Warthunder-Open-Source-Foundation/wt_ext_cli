...
pub fn unpack_vromf(args: &ArgMatches) -> Result<()> {
	...
	let export_meta = *args
		.get_one::<bool>("export_meta")
		.context("Invalid argument: export_meta")?;
...
fn parse_and_write_one_vromf(
	...
	let vromf_name_for_meta = vromf_name.clone();
	...
	let meta = parser.metadata().clone();

	parser.unpack_all_with_writer(format, should_override, writer, true, filter, continue_mode)?;

	if export_meta {
		let meta_path = output_dir.join(&vromf_name_for_meta).join("meta.toml");
		fs::create_dir_all(meta_path.parent().ok_or(CliError::InvalidPath)?)?;
		let header = format!(
			"# Generated by wt_ext_cli {}\n# Commit: {}\n",
			crate::GIT_TAG,
			crate::COMMIT_HASH,
		);
		let toml_str = format!("{header}{}", toml::to_string_pretty(&meta)?);
